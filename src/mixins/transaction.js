...
export default {
  methods: {
     ...
     * @param {number} timestamp P2P special message timestamp. If coin Tx is not known yet, set its timestamp to ADM rich message timestamp. Later it will be updated
     */
      ...
      if (admSpecialMessage) {
        ...
        if (type === Cryptos.ADM || type === 0 || type === 8 || type === 'message') {
          if (admSpecialMessage.status === TS.REGISTERED) {
            // If it's a message, getChats() in P2P-api.js will update height and confirmations later,
            ...
            if (type === 'message') {
              ...
              // Currently, we don't update confirmations for direct transfers, see getChats() in P2P-api.js
