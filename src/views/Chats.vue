          ...
          >
            ...
            >
              <chat-preview
                ...
                :is-P2P-chat="isP2PChat(transaction.contactId)"
...
export default {
  ...
  computed: {
    ...
    messages () {
      ...
      if (!this.noMoreChats && clonedLastMessages.length > 25) {
        const lastNotP2PChat = lastMessages.map(msg => this.isP2PChat(msg.contactId)).lastIndexOf(false)
        if (lastNotP2PChat) {
          clonedLastMessages.splice(lastNotP2PChat + 1, 0, { loadingSeparator: true, userId: 'loadingSeparator', contactId: 'loadingSeparator' })
        }
      }
  ...
  methods: {
    ...
    isP2PChat (partnerId) {
      return this.$store.getters['chat/isP2PChat'](partnerId)
    },
    onError (message) {
