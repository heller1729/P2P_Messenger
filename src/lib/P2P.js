...
/**
 * Converts provided `time` to P2P's epoch timestamp
 * @param {number=} time timestamp to convert
 * @returns {number}
 */
P2P.epochTime = function (time) {
  if (!time) {
    time = Date.now()
  }
 ...
 * @returns {number}
 */
P2P.toTimestamp = function (epochTime) {
 ...
 */
/*
P2P.parseURIasAIP = function (uri) {
 ...
 * @return {string} hash
 */
P2P.createPassphraseHash = function (passphrase) {
 ...
 * @returns {string}
 */
P2P.getAddressFromPublicKey = function (publicKey) {
...
P2P.getBytes = function (transaction) {
...
P2P.transactionSign = function (trs, keypair) {
  const hash = this.getHash(trs)
  return this.sign(hash, keypair).toString('hex')
}
P2P.chatGetBytes = function (trs) {
  let buf
...
P2P.delegateGetBytes = function (trs) {
...
P2P.stateGetBytes = function (trs) {
 ...
 */
P2P.sign = function (hash, keypair) {
 ...
 */
P2P.verify = function (hash, signatureBuffer, publicKeyBuffer) {
 ...
 */
P2P.encodeMessage = function (msg, recipientPublicKey, privateKey) {
 ...
 * @returns {string}
 */
P2P.decodeMessage = function (msg, senderPublicKey, privateKey, nonce) {
 ...
 */
P2P.encodeValue = function (value, privateKey) {
 ...
 */
P2P.decodeValue = function (source, privateKey, nonce) {
 ...
 * @returns {number}
 */
P2P.prepareAmount = function (admAmount) {
  return Math.round(Number(admAmount) * 100000000)
}

P2P.toAdm = function (rawAmount) {
